...
coin_ids = csv.reader(open(os.path.dirname(__file__) + '../data/csv/final_coin_ids.csv'), delimiter=",")
keys = coin_ids.next()
...
#for i, fn in enumerate(os.listdir(os.path.dirname(__file__)  + './../data/images/thumbs_front')[:5000]):
...
for i, coin_data in enumerate(coin_ids):
  ...
  sprite_sheet_specs.append([x_pos, y_pos, new_thumb_width, new_thumb_height])
...
# specs_file = open(os.path.dirname(__file__) + '../data/csv/coins_sprites_specs.csv', 'w')

# writer = csv.writer(specs_file)
# writer.writerows(sprite_sheet_specs)
# specs_file.close()